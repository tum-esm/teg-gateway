...
while building on top of field-tested software. The TEG-gateway leverages the ThingsBoard IoT platform (@ThingsBoard), a 
...
The flexible design of the TEG gateway, combined with the ThingsBoard IoT platform, enables users to configure customized 
...
The TEG-Gateway software is written in Python (@Python). It follows a modular design, encapsulating independent
...
The TEG-Gateway receives telemetry data from the Controller Software via a local sqlite database (@SQLite), which is used to
...
The OTA update feature (2) enables users to remotely deploy new versions of the Controller Software to the TEG-Gateway device, for
...
The TEG-Gateway has been validated in a real-world scientific deployment within the ICOS Cities project. It has 
